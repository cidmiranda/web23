...
contract JoKenPo {
    ...
    function secretFunction() public view returns (string memory) {
        require(owner==msg.sender, "You don't have permisssions to do this.");
        return "olele";
    }
    ...
    function finishGame(string memory newResult, address winner) private {
        address contractAdrress = address(this);
        payable(winner).transfer((contractAdrress.balance / 100) * 90);
        owner.transfer(contractAdrress.balance);
